...
public class clothingAdapter extends BaseAdapter {
    ...
    public clothingAdapter(Activity context, ArrayList<String> shopname, ArrayList<String> shopaddress, ArrayList<String> imageurl) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        {
            image.setBackgroundResource(R.color.tw__transparent);
            Picasso.with(context).load(imageurl.get(position)).into(image);
        }
        else
        {
            Picasso.with(context).load(R.drawable.account_logo).into(image);
        }
        return convertView;
    }
}
